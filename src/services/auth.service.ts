...
export class AuthService {
  ...
  async refresh(refreshPayload: RefreshTokenPayload) {
    const valid = await this.redisService.isValidJti(refreshPayload.sub, refreshPayload.jti);
    if (!valid) throw new UnauthorizedError("Refresh token expirado ou revogado");

    await this.redisService.removeJti(refreshPayload.sub, refreshPayload.jti);

    const user = await this.userService.findById(refreshPayload.sub);

    const accessToken = generateAccessToken(refreshPayload.sub, user.name);
  ...
  async logoutAll(payload: RefreshTokenPayload) {
    const { sub } = payload;
